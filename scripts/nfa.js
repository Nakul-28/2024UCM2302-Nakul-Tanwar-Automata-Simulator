document.addEventListener('DOMContentLoaded', () => {
    ...
    // ================================================================
    // FEATURE 3: TRANSITION TABLE (NFA — cells may contain multiple states)
    // ================================================================
    function buildTransitionTable() {
        ...
        const rows = appState.nodes.map(node => {
            const cells = symbols.map(sym => {
                // NFA: collect epsilon closure of all reachable states on this symbol
                const directTargets = new Set();
                appState.edges
                    .filter(e => e.from === node.id && e.symbols.includes(sym))
                    .forEach(e => directTargets.add(e.to));

                // Apply epsilon closure
                const closure = getEpsilonClosure(Array.from(directTargets));
                const labels = closure.map(id => {
    ...
    function showTransitionTable() {
        ...
        html += '<th>State</th>';
        symbols.forEach(s => { html += `<th>${s === 'ε' ? 'ε' : s}</th>`; });
        html += '</tr></thead><tbody>';
        ...
        html += '</tbody></table>';
        html += '<p style="margin-top:12px;font-family:\'JetBrains Mono\',monospace;font-size:0.72rem;color:var(--text-secondary);">→ = start state &nbsp;|&nbsp; (q) = accept state &nbsp;|&nbsp; ∅ = no transition &nbsp;|&nbsp; ε-closure applied</p>';
