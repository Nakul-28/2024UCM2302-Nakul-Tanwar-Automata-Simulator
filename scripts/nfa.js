document.addEventListener('DOMContentLoaded', () => {
    ...
    function resetSim() {
        ...
        if (appState.sim.activeStates.length > 0) {
            pushLiveLog(`Initial active states (after ε-closure): ${formatStateSet(appState.sim.activeStates)}`);
        } else {
    ...
    // Re-bind delete buttons with snapshot capture
    document.getElementById('btn-delete-state').addEventListener('click', () => {
        if (appState.selectedNodeId) {
            saveSnapshot();
            appState.nodes = appState.nodes.filter(n => n.id !== appState.selectedNodeId);
    ...
    // Capture phase: save snapshot before adding state or finalizing edge
    canvas.addEventListener('mousedown', (e) => {
    ...
    // ================================================================
    // FEATURE 3: TRANSITION TABLE EDITOR (NFA)
    // ================================================================
        ...
        states.forEach(state => {
            ...
            symbols.forEach(sym => {
                const targets = appState.edges
                    .filter(e => e.from === state.id && e.symbols.includes(sym))
                    .map(e => {
                        const node = appState.nodes.find(n => n.id === e.to);
                        return node ? node.label : e.to;
                    });
                transitions[state.id][sym] = Array.from(new Set(targets)).join(', ');
            });
        });
    ...
    function renderTransitionEditor() {
        ...
        html += '<input type="text" id="table-new-symbol" placeholder="New symbol (a, b, ε)">';
        ...
        tableEditor.states.forEach(state => {
            ...
            html += `<td><input type="checkbox" class="table-state-start" data-state-id="${state.id}" ${state.isStart ? 'checked' : ''}></td>`;
            ...
            tableEditor.symbols.forEach(sym => {
                ...
                html += `<td><input type="text" class="table-transition-input" data-from="${state.id}" data-symbol="${sym}" placeholder="q1, q2" value="${value}"></td>`;
            });
        ...
        html += '</tbody></table>';
        html += '<div class="table-editor-hint">NFA rule: each cell may list multiple targets separated by commas. Leave blank to delete transition.</div>';
    ...
    function removeStateFromModel(stateId) {
        ...
        tableEditor.states.forEach(s => {
            tableEditor.symbols.forEach(sym => {
                const raw = tableEditor.transitions[s.id]?.[sym] || '';
                const cleaned = raw
                    .split(',')
                    .map(v => v.trim())
                    .filter(v => v.length > 0)
                    .filter(v => tableEditor.states.some(t => t.label === v));
                tableEditor.transitions[s.id][sym] = cleaned.join(', ');
            });
        });
    }
    ...
    function bindTransitionEditorEvents() {
        ...
        document.querySelectorAll('.table-state-start').forEach(input => {
            input.addEventListener('change', () => {
                const state = tableEditor.states.find(s => s.id === input.dataset.stateId);
                if (state) state.isStart = input.checked;
            });
        });
    ...
    function applyTransitionEditorToDiagram() {
        ...
        cleanedStates.forEach(state => {
            tableEditor.symbols.forEach(sym => {
                const rawTargets = (tableEditor.transitions[state._tableId]?.[sym] || '').trim();
                ...
                targets.forEach(targetLabel => {
                    ...
                    if (!targetState) {
                        errors.push(`Missing target state '${targetLabel}' for ${state.label} with symbol '${sym}'`);
                        return;
                    }
